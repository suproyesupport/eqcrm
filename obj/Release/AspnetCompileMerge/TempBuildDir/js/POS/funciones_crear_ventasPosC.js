...
// -- RECORREMOS LA TABLA PARA VALIDAR CODIGOS INGRESADOS DUPLICADO
function validarCodigoIngresado() {

    var codigo = document.getElementById("id_codigo").value;
    var cantidad = document.getElementById("cantidad").value;
    var descto = document.getElementById("descto").value;
    ...
    $("#tabladetalle tr").each(function () {
        ...
        itemProducto.codigo = tds.filter(":eq(0)").text();
        itemProducto.cantidad = tds.filter(":eq(1)").text();
        itemProducto.producto = tds.filter(":eq(2)").text();
        itemProducto.precio = tds.filter(":eq(3)").text();
        itemProducto.descto = tds.filter(":eq(4)").text();
        itemProducto.subtotal = tds.filter(":eq(5)").text().replace(",", "");
        
        items.push(itemProducto);

        if (codigo == itemProducto.codigo) {

            itemProducto.cantidad = (Number(itemProducto.cantidad) + Number(cantidad));
            itemProducto.descto = (parseFloat(itemProducto.descto) + parseFloat(descto));
            itemProducto.subtotal = ((Number(itemProducto.cantidad) * Number(itemProducto.precio)) - parseFloat(itemProducto.descto));

            if (descto >= 1) {
                var celda0 = "<span id='mySpan' style='color: red;'>" + itemProducto.codigo + "</span>";
                var celda1 = "<span id='mySpan' style='color: red;'>" + itemProducto.cantidad + "</span>";
                var celda2 = "<span id='mySpan' style='color: red;'>" + itemProducto.producto + "</span>";
                var celda3 = "<span id='mySpan' style='color: red;'>" + itemProducto.precio + "</span>";
                var celda4 = "<span id='mySpan' style='color: red;'>" + itemProducto.descto + "</span>";
                var celda5 = "<span id='mySpan' style='color: red;'>" + itemProducto.subtotal + "</span>";

            } else {
                var celda0 = "<span id='mySpan' style='color: black;'>" + itemProducto.codigo + "</span>";
                var celda1 = "<span id='mySpan' style='color: black;'>" + itemProducto.cantidad + "</span>";
                var celda2 = "<span id='mySpan' style='color: black;'>" + itemProducto.producto + "</span>";
                var celda3 = "<span id='mySpan' style='color: black;'>" + itemProducto.precio + "</span>";
                var celda4 = "<span id='mySpan' style='color: black;'>" + itemProducto.descto + "</span>";
                var celda5 = "<span id='mySpan' style='color: black;'>" + itemProducto.subtotal + "</span>";

            }

            tds.filter(":eq(0)").html(celda0);
            tds.filter(":eq(1)").html(celda1);
            tds.filter(":eq(2)").html(celda2);
            tds.filter(":eq(3)").html(celda3);
            tds.filter(":eq(4)").html(celda4);
            tds.filter(":eq(5)").html(celda5);
        ...
        parseFloat(itemProducto.subtotal);
        total = parseFloat((parseFloat(total) + parseFloat(itemProducto.subtotal))).toFixed(2);
        $("#total").text(formatNumber.new(parseFloat(total).toFixed(2)));
    });
...
function AddDetallePosc() {
    ...
    if (descuento >= 1) {
        ...
        var celda1 = "<span id='mySpan' style='color: red;'>" + $("#id_codigo").val() + "</span>";
        var celda2 = "<span id='mySpan' style='color: red;'>" + $("#cantidad").val() + "</span>";
        var celda3 = "<span id='mySpan' style='color: red;'>" + $("#producto").val() + "</span>";
        var celda4 = "<span id='mySpan' style='color: red;'>" + formatNumber.new(document.getElementById("precio").value) + "</span>";
        var celda5 = "<span id='mySpan' style='color: red;'>" + formatNumber.new(descuento_) + "</span>";
        var celda6 = "<span id='mySpan' style='color: red;'>" + formatNumber.new($("#tsubtotal").text()) + "</span>";

    } else {

        var celda1 = "<span id='mySpan' style='color: black;'>" + $("#id_codigo").val() + "</span>";
        var celda2 = "<span id='mySpan' style='color: black;'>" + $("#cantidad").val() + "</span>";
        var celda3 = "<span id='mySpan' style='color: black;'>" + $("#producto").val() + "</span>";
        var celda4 = "<span id='mySpan' style='color: black;'>" + formatNumber.new(document.getElementById("precio").value) + "</span>";
        var celda5 = "<span id='mySpan' style='color: black;'>" + formatNumber.new(descuento_) + "</span>";
        var celda6 = "<span id='mySpan' style='color: black;'>" + formatNumber.new($("#tsubtotal").text()) + "</span>";
    }


    

    if (validarCodigoIngresado() != "true") {
        ...
        cell4.innerHTML = celda4;
        cell5.innerHTML = celda5;
        cell6.innerHTML = celda6;
...
function enterCantidad(event) {
    if (event.keyCode == 13) {
        document.getElementById('descto').focus();
    }
}

function enterDescto(event) {
    if (event.keyCode == 13) {
        document.getElementById('btnagregar').focus();
    }
}
...
function SaveDataPostPosc() {

    $('#DatosClie').modal('show');
...
function focusCantidadPosC() {

    var element = document.getElementById("precio");

    if (element.disabled) {
        CalcularValoresGenericoPosC();
        validarDesctoVacio();
        //document.getElementById("cantidad").focus();
...
function CalcularValoresGenericoPosC() {
...
//////////vamos a poner esto y regresamos
function ObtieneInfoTablaPosc() {
    ...
    for (var n = 1; n < tabla; n++) {
        ...
        if (nContador <= nDetalle) {
            var fila = $("#tabladetalle tr:nth-child(" + valor + ")").html();  //recupera desde la segunda fila de la tabla
            var datos = fila.replace(/<td>/g, '~');
            //datos = datos.replace('<span id="mySpan" style="color: red;">', '');
            //datos = datos.replace(/<span id="mySpan" style="color: black;">/, '');
            //datos = datos.replace(/<span id="mySpan" style="color: red;">/, '');
            datos = datos.replace(/[/]/g, '');
            datos = datos.replace(/<td>/g, '');
...
function SaveDataPost() {
    ...
    if (formaPago == 1) {
        ...
        $.ajax({
            ...
            dataType: "text",
            contentType: "application/x-www-form-urlencoded",
            url: "/POS/Operaciones/InsertarDocumentoPosc",
            data: { cdetalle },
            success: function (response) {
...
function SaveDataPostC() {

    var cdetalle = btoa(ObtieneInfoTablaPosc());
...
function finalizarDataPostC() {

    cdetalle = ObtieneInfoTablaPosc();
    formaPago = $("#tipoFormaPago").val();
